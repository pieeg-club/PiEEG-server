...
The `device` field is the active hardware profile, identical to the CLI
`--device` value: `pieeg8`, `pieeg16`, `ironbci8`, or `ironbci32`. Use it to
disambiguate devices that share a channel count and sample rate (e.g.
`pieeg8` vs `ironbci8`).

The `channel_labels` field is the authoritative index→electrode mapping:
`channel_labels[i]` names `channels[i]` in every EEG data frame, and its
length always equals `channels`. It is **optional** and only present for
devices with a fixed 10-20 montage (`pieeg8`, `pieeg16`). It is omitted for
the IronBCI ear-EEG headset (`ironbci8`) and `ironbci32`, which have no
authoritative on-the-wire label order — clients should fall back to
positional indices in that case.

Both fields are optional additions; existing clients that ignore unknown keys
are unaffected.
