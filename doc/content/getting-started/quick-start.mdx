...
<Callout type="info">
  IronBCI-32 requires the serial extra: `pip install pieeg-server[ironbci32]`. Pure-Python pyserial driver — no BrainFlow required. 32 channels at ~500 SPS over USB CDC, FreeEEG wire protocol at 921600 baud.
</Callout>
...
pieeg-server --device ardeeg8 --udp-ip 192.168.1.50  # bind a specific interface
```

<Callout type="info">
  ardEEG needs no extra dependency — UDP uses the Python standard library. Flash the [ardEEG firmware](https://github.com/pieeg-club/ardEEG) with your WiFi SSID/password and set its destination IP to the machine running pieeg-server. The board streams 8 channels at 250 Hz in 50-sample UDP bursts. Only one program can bind the UDP port at a time, so close the reference receiver before starting the server.
</Callout>
