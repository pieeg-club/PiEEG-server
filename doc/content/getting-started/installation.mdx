...
The [ardEEG](https://github.com/pieeg-club/ardEEG) board (Arduino Uno WiFi + ADS1299) streams EEG over WiFi using UDP. No extra dependency is required — UDP uses the Python standard library:
...
<Callout type="info">
  Flash the ardEEG firmware with your WiFi SSID/password and set its destination IP to the machine running pieeg-server. The board streams 8 channels at 250 Hz in 50-sample UDP bursts. Only one program can bind the UDP port at a time.
</Callout>
